                       ...
                       uint8_t type)
{
    ...
    //printf("_check_mem : addr %d, end %d\n", addr, end);

    for (const rbpf_mem_region_t *region = &rbpf->stack_region; region; region = region->next) {
        //printf("_check_mem : region->start %d , region->end %d\n", (intptr_t)(region->start), (intptr_t)(region->start + region->len));
        if ((addr >= (intptr_t)(region->start)) &&
            (end <= (intptr_t)(region->start + region->len)) &&
            (region->flags & type)) {
            //printf("_check_mem : OK\n");
            return true;
        }
    }

    //printf("_check_mem : KO\n");
    return false;
}
